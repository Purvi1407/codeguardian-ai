...
This is the persistence layer three earlier phases already pointed at without building: Module 4's original design notes called `dismissed` findings "the seed of a future feedback loop (Module 5+, not built)"; Phase 6 deferred dismiss/resolve/history specifically because they'd "substantially overlap with Phase 9's 'feedback store'"; Phase 8's baseline scanning was explicitly the no-persistence-needed stopgap *because* this didn't exist yet. It exists now.

**What changed:**
- `app/services/feedback_store.py` — new. SQLite-backed (consistent with this project's "reach for the simplest thing that works" pattern — no other datastore exists yet, and a single embedded database file needs no new infrastructure decision). Every `/validate` call now records each verdict (rule_id, file, function, severity, verified/dismissed, confidence, timestamp) and a scan-level summary (repository, branch, candidate/verified counts, risk score, timestamp).
- `app/schemas/feedback.py` — new. `RuleStat` (per-rule totals, dismissal rate, a `needs_review` flag) and `ScanRecord` (one past scan).
- `app/api/stats.py` — new, two read-only endpoints:
  - `GET /stats/rules` — dismissal rate per rule across every recorded validation ever, with `needs_review=True` once a rule has enough samples (`CODEGUARDIAN_MIN_SAMPLE_SIZE`, default 5) and a high enough dismissal rate (`CODEGUARDIAN_DISMISSAL_THRESHOLD`, default 0.5) to be worth a person looking at.
  - `GET /stats/scans?repository=...&limit=...` — scan history, most recent first — the raw data a "risk score over time" trend view would read from.
- `app/api/validate.py` — records to the feedback store after every scan, wrapped so a storage failure degrades to "this scan's feedback wasn't recorded" rather than breaking the response someone is actively waiting on — the same best-effort contract Phase 5's cache already established for exactly this reason.
- **New test files** — `tests/test_feedback_store.py` (17 tests), `tests/test_stats_api.py` (7 tests), plus additions to `tests/test_api_filters.py` confirming `/validate` actually records and that a recording failure doesn't break the response. **28 new tests.**
- **A genuine bug fix, unrelated to this phase's actual feature work**: `.gitignore` had `backend/cache/` for Phase 5's AI-validation cache, but `core/config.py`'s `BASE_DIR` resolves to the *repository root*, not `backend/` — the real cache file has been living at `<repo root>/cache/ai_validation_cache.json` this whole time, a path the gitignore entry never actually matched. Caught it while deciding where this phase's new SQLite file should live (same `BASE_DIR`-relative pattern) and fixed both the actual bug and the misleading trail: `.gitignore` now correctly lists `cache/` and `data/` at the repo root.

**369 total tests now**, all passing, verified in a clean venv matching CI exactly, and confirmed the app boots with both new routes registered (`/stats/rules`, `/stats/scans`) alongside every existing one. 100% coverage on all three new files.

### The bug that almost shipped twice

While writing `get_rule_stats()`'s "needs review" thresholds, I made the *exact same mistake* Phase 5's README already documents catching and fixing: `CODEGUARDIAN_MIN_SAMPLE_SIZE` and `CODEGUARDIAN_DISMISSAL_THRESHOLD` were first written as module-level constants (`= int(os.getenv(...))`), evaluated once at import time — meaning a test using `monkeypatch.setenv` to change them mid-test-run would silently have no effect, since the constant was already fixed before the test even started. A test failed unexpectedly during development (`needs_review` came back `False` when it should've been `True`), which is exactly how the Phase 5 instance of this same bug was originally caught too. Fixed the same way: read the env var fresh via a small function each time, matching the pattern `analyzer/python_rules.py`'s `_disabled_rule_ids()` (Phase 4) and `ai/validator.py`'s `_fallback_model()` (Phase 5) both already use.

Worth naming plainly rather than quietly fixing: knowing about a bug class doesn't automatically prevent writing it again — it just makes it faster to recognize and fix once a test catches it. That's the actual argument for the test suite this whole project has been built around, not just "tests exist," but "tests catch things, including the same mistake twice."

### Design notes (for defending decisions)

- **Why SQLite instead of the file-based JSON approach Phase 5's cache and Phase 8's baseline both use?** Those are single-key-lookup or whole-file-snapshot use cases — a cache hit/miss, or "is this fingerprint in the baseline." This phase needs actual aggregation (`GROUP BY rule_id`, `COUNT`, `SUM`, `ORDER BY timestamp DESC LIMIT N`) across a growing, unbounded history of records — the kind of thing a JSON file re-read-and-recomputed on every request would get slower at linearly forever, while SQLite does it with an index in roughly constant time. Still a single file, still zero external services — just the right tool once the access pattern is genuinely relational instead of key-value.
- **Why is recording best-effort (silently swallowed on failure) rather than surfaced as a warning in the API response?** Consistent with Phase 5's cache: the feedback store is a secondary concern to the primary thing `/validate` exists to do (return validated findings). A person waiting on a scan result shouldn't get a degraded or confusing response because analytics storage had a problem — but this is a real tradeoff, not a free lunch: a silent failure here means a repeated storage problem (e.g. a full disk) could go unnoticed for a while. Worth revisiting with actual logging/alerting if this were headed to a real production deployment rather than a take-home project's continued build-out.
- **Why does `needs_review` only ever get read by a person via `/stats/rules`, rather than automatically feeding back into `CODEGUARDIAN_DISABLED_RULES` (Phase 4)?** This is the deliberate, named boundary of "feedback loop" in this phase: surfacing a signal, not acting on it autonomously. A rule with a high dismissal rate might genuinely need tightening (a real signal) — or it might be getting dismissed correctly and often precisely *because* the AI validation layer is doing its job on a rule that's intentionally broad (e.g. `path-traversal-open`, which flags any dynamic path by design). Auto-disabling based on dismissal rate alone can't tell those two cases apart; a person reading the flagged rule's actual dismissed findings can. Same "candidate generator, not an autonomous decision-maker" philosophy the whole rule engine has used since Module 3.
- **Known limitation, stated plainly**: Render's default web service filesystem is ephemeral — a redeploy can wipe both the Phase 5 cache and this phase's SQLite file. Fine for the cache (worst case: re-pay for some AI calls). Real data loss for accumulated feedback/stats on the current deployment. A persistent disk (Render offers this as an add-on) or migrating to a hosted database would fix it — not done here, since it's an infrastructure/cost decision for whoever's actually running this in production, not a code change this phase should make unilaterally.

## Phase 10 — Production Quality: the final phase

This roadmap's last stop, and it's a genuinely different kind of work than everything before it — not new capabilities, but making the existing ones trustworthy to actually run somewhere real. Every item below was chosen because a concrete gap was found and fixed, not because the roadmap item existed; see `ARCHITECTURE.md` (new this phase) for a full map of the finished system.

**What changed:**

- **A real bug fix: repository size limits were never enforced.** `core/config.py` has defined `MAX_REPO_SIZE_MB` since Module 1, and it's even referenced in a comment in the Phase 4 README — but nothing in `repo_processor.py` ever actually checked it. A repo of any size would clone successfully. Fixed: `clone_repository()` now measures the cloned directory and rejects (with cleanup) anything over the limit. Checked *after* the clone, not before — there's no reliable way to know a shallow-clone's checked-out size without actually fetching it — a real, named limitation, not a silent gap.
- **A real security gap: this tool could re-leak the secrets it finds.** The `hardcoded-secret` rule's `snippet` field contained the literal secret value from the scanned repo — which then flowed into the AI validation prompt (sent to a third party, OpenAI), the Phase 5 on-disk cache, and the Phase 9 SQLite feedback store, all indefinitely. Fixed at the source: `analyzer/python_rules.py` and `js_ts_rules.py` now redact the literal secret value in the snippet (`api_key = "<redacted>"`) while keeping the finding fully actionable — variable name, function, file, and line are all still there.
- **Structured logging, where there was none.** `app/core/logging_config.py` (new) — every clone attempt, analysis run, and validation now logs at INFO/WARNING/ERROR, configurable via `CODEGUARDIAN_LOG_LEVEL`. Deliberately never logs full Finding snippets, even post-redaction — logs are a second place sensitive data could sit around, so the discipline from the redaction fix above applies here too, not just to the API response.
- **CORS was wide open with a comment that said so.** `main.py` had `allow_origins=["*"]` next to a comment reading "Tighten this before deploying" — present since the very first version of the file, never acted on. Now configurable via `CODEGUARDIAN_CORS_ORIGINS` (comma-separated), defaulting to `*` for continuity but actually fixable with one env var instead of a code change.
- **Rate limiting, where there was none.** `app/core/rate_limit.py` (new) — a simple in-memory sliding-window limiter on the three expensive endpoints (`/scan`, `/analyze`, `/validate`), configurable via `CODEGUARDIAN_RATE_LIMIT_PER_MINUTE`. Explicitly documented as single-process, not distributed — see design notes below.
- **`scripts/benchmark.py`** (new) — a reusable, re-runnable benchmark, not just numbers pasted into this README. Against `pallets/flask` (83 Python files, ~18.3k lines): ~0.45–0.48s median. Against `appsecco/dvna` (12 JS files via tree-sitter): ~0.20–0.21s median. Anyone can re-run `python -m scripts.benchmark /path/to/repo` themselves.
- **A genuine full-stack integration test** (`tests/test_integration.py`) — distinct from every prior API test, which mocks the analysis pipeline itself to isolate what it's testing. This one mocks *only* the network clone boundary and lets parsing, analysis, filtering, summary computation, and feedback-store persistence all run for real against a real local git repo with real vulnerable code in two languages.
- **`ARCHITECTURE.md`** (new) — a from-scratch reader's map of the finished system: the pipeline end to end, the two entry points (API and CLI) and why they share code, the two persistence mechanisms and why their formats differ, and an explicit "what's not here, on purpose" section.
- **New test files** — `tests/test_repo_processor.py` (30 tests — this file had *zero* dedicated tests before this phase, at 32% coverage), `tests/test_secret_redaction.py` (7), `tests/test_main.py` (7), `tests/test_rate_limit.py` (7), `tests/test_logging_config.py` (6), `tests/test_integration.py` (4), `tests/test_scan_service.py` (10). **71 new tests.**

**440 total tests now**, all passing, verified in a clean venv matching CI exactly. **97% coverage across the entire `app/` package** — every module, not just the ones touched this phase.

### A real test-isolation bug, caught and fixed during this same phase

Adding the rate limiter broke the existing test suite the moment it landed — not because the limiter was wrong, but because its in-memory counters live for the lifetime of the imported `app.main` module, which Python caches for the entire pytest session. Dozens of existing tests across `test_api_filters.py` and others POST to `/scan`/`/analyze`/`/validate` far more than the sensible production default of 10 requests/minute, and they all share that one cached module instance. Fixed with a session-scoped autouse fixture in `conftest.py` that sets a generous rate limit for the whole test session by default, while `test_rate_limit.py`'s own tests explicitly `monkeypatch` a lower value against a freshly `importlib.reload`-ed app instance to test the real limiting behavior in isolation. Confirmed fixed by running the full suite three times in a row with no flakiness.

### Design notes (for defending decisions)

- **Why is repo-size enforcement post-clone instead of pre-clone?** GitHub's API-reported repository size is compressed and doesn't account for `--depth 1`'s history truncation — there's no reliable number to check *before* spending the clone time. This means an oversized repo still costs bandwidth and time before being rejected. A pre-flight check using GitHub's API (a separate network call, with its own rate limits and auth considerations) would reduce but not eliminate this, and wasn't judged worth the added complexity and second external dependency for what's fundamentally a best-effort safety limit, not a hard guarantee.
- **Why redact only `hardcoded-secret`, not `insecure-random-token` (Phase 4) too?** `insecure-random-token` flags a call to `random.randint()`/`Math.random()` — a *pattern*, not a captured literal value. There's no secret string to redact in the first place; the snippet just shows `token = random.randint(...)`, which reveals nothing sensitive. Redaction only applies where there's genuinely something to redact.
- **Why is the rate limiter in-memory rather than reaching for a library like slowapi or a Redis-backed limiter?** This project runs as one process per deployment right now — the same reality `repo_processor.py`'s size check and `feedback_store.py`'s SQLite file already live with (see that phase's notes on Render's single-instance, ephemeral-filesystem deployment). A distributed limiter solves a coordination problem across multiple processes/instances that doesn't exist yet. Named explicitly as a real limitation (see the module's own docstring) rather than silently assumed away — `uvicorn --workers 4` would allow up to 4x the configured limit, since each worker keeps its own counters, and that's the honest boundary of what this implementation covers.
- **Why does the shared-counter-across-endpoints behavior in the rate limiter (hitting `/scan`'s budget also affects `/analyze`) not get fixed to be per-path?** It's a legitimate design choice, not an oversight — caught and named explicitly via a dedicated test (`test_different_endpoints_share_one_counter_per_client`) rather than silently shipped either way. A client hammering *any* combination of the three expensive endpoints is still hammering the service; a per-path limit would let someone bypass the overall limit by round-robining across `/scan`, `/analyze`, and `/validate`. Worth revisiting if usage patterns ever show a real need for independent budgets per endpoint.
- **What Phase 10 explicitly did NOT attempt**: parallel/incremental scanning beyond what Phase 8's `--changed-only` already provides (genuine parallelization needs profiling first to know where time actually goes, not a guess at what to parallelize), and full API request/response schema examples in the OpenAPI docs beyond what FastAPI already auto-generates from the Pydantic models (a real but lower-value polish item compared to the security/reliability gaps found and fixed above). Named here rather than silently skipped, the same way every deferred item across all 10 phases has been.

---

This closes out the original 10-phase roadmap. 440 tests, 97% coverage, ten independently-verified, independently-documented phases — tree-sitter parsing, bounded cross-function taint tracking, 20 security rules across two languages, AI validation with caching and retry/fallback handling, filtering and a dashboard, a CLI with SARIF output and CI integration, a real feedback store, and the production-hardening pass in this phase. Every phase found and fixed at least one real, concrete problem along the way — not just added features on top of a static foundation. `ARCHITECTURE.md` is the fastest way to see the whole shape of it; this README is the record of how it got built and why each decision was made the way it was.
